 ...
 * 
	...
	<bean id="mysqlDataBaseDataOperation"
		class="org.yelong.support.orm.mybaits.mapper.DefaultMyBatisBaseDataBaseOperation">
		<constructor-arg index="0" ref="mysqlSqlSession" /><!-- 集成Mybatis。这里使用SqlSession的bean -->
	</bean>
	...
	<bean id="mysqlModelService"
		...
		<constructor-arg index="0" ref="mysqlModelConfiguration"></constructor-arg>
	...
	<bean id="oracleDataBaseDataOperation"
		class="org.yelong.support.orm.mybaits.mapper.DefaultMyBatisBaseDataBaseOperation">
		<constructor-arg index="0" ref="oracleSqlSession" /><!-- 集成Mybatis。这里使用SqlSession的bean -->
	</bean>

	<bean id="oracleModelConfiguration"
		class="org.yelong.mybatis.spring.ModelConfigurationAutoConfigure"><!-- 可以注入自定义的一些对于Model管理的类来实现自定义管理Model -->
		<property name="dialectName" value="oracle"></property>
	</bean>

	<bean id="oracleModelService"
		class="org.yelong.mybatis.spring.MyBatisModelService"><!-- 自定义实现MyBatisModelService 实现类可以自定义实现Model的操作前后执行一些操作 -->
		<constructor-arg index="0" ref="oracleModelConfiguration"></constructor-arg>
		<constructor-arg index="1"
